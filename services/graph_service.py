...
def build_network_data(results: list[dict], center_label: str = "Narrative") -> dict:
    ...
    try:
        ...
        # Use save_graph so PyVis copies the lib directory!
        net.save_graph("exports/network.html")
        
        # PyVis creates 'lib' in the current working directory, we need it in 'exports/lib'
        if os.path.exists("lib") and not os.path.exists("exports/lib"):
            shutil.copytree("lib", "exports/lib")
        
        with open("exports/network.html", "r", encoding="utf-8") as f:
            html = f.read()
            
        inject = """
        network.on("click", function (params) {
            if (params.nodes.length > 0) {
                window.parent.postMessage({type: 'node_click', node: params.nodes[0]}, '*');
            }
        });
        """
        html = html.replace("return network;", f"{inject}\nreturn network;")
        
        with open("exports/network.html", "w", encoding="utf-8") as f:
            f.write(html)
